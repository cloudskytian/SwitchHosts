...
| `core:webview:default` | ✓ | ✓ | ✓ | 三窗口都允许 DevTools（`allow-internal-toggle-devtools` 已包含在 `core:webview:default` 的默认集中，便于调试托盘/查找窗口） |
...
v5 首版规划 13 个 command 分组，共 ~50 条命令。下表按组列出：
...
| command | `main` | `find` | `tray` | 说明 |
| --- | --- | --- | --- | --- |
| `get_hosts_content` | ✓ | ✓ | × | 读某节点正文（`find` 窗口搜索时读取） |
| `set_hosts_content` | ✓ | × | × | 写某节点正文；**`find` 不直接持有写权限** |
| `get_system_hosts` | ✓ | × | × | 读 `/etc/hosts` 原始内容 |
| `get_path_of_system_hosts` | ✓ | × | × | 展示系统 hosts 路径 |

**查找/替换写回路径**：`find` 窗口执行替换后不直接调用 `set_hosts_content`，而是通过 Tauri event 把替换结果发送给 `main` 窗口，由 `main` 窗口代为调用 `set_hosts_content` 完成写入。这样 `find` 窗口的能力集保持为"只读 + 发事件"，与"辅助窗口默认收紧"原则一致。
...
| 文件 | `windows` | 涵盖能力 |
| --- | --- | --- |
| `main.json` | `["main"]` | G1–G12 全集（按实际使用为准），core plugin 全部允许项 |
| `find.json` | `["find"]` | G1 + G2 + G4（仅 `get_hosts_content`，不含 set）+ G7，core plugin 裁剪 |
| `tray.json` | `["tray"]` | G1 + G2 + G5（仅 apply/toggle）+ G9（focus/hide main），core plugin 裁剪 |
